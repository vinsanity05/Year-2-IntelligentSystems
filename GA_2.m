...
%Firstly my task is to seed the random number generator with '666' putting the following in a useful place. 
%rng('default'); rng (666);  Then Improve performance. So firstly, i added
%underneath the fprintf(1,'Hit any key to generate a population of %.0f
%chromosomes.\n',nind); and added the seed of the random number generator
%with 666 in this place and this is what I included underneath > pause rng('default');( rng(666); 
%When running this, the outcome came that the red dots were placed on the
%little peak and no red dots on the big peak. So to improve performance I
%experimeted and changed the size of the chromosomes population from 6 to
%15. This made a better performance although some dots were still on the
%little peak and sometimes out of the area of the two. Furthermore,
%I also changed the mutation from 0.005 to 0.001. This made it better to
%improve performance. The outcome was that all the red dots were placed in
%the big peak and the performance graph also shows that after 11
%generations the best and average are in line which shows the best
%optimisation and best performance. 
...
nind=15;     % Size of a chromosome population changed to this 
...
Pm=0.001;   % Mutation probability changed to this 
...
pause
rng('default'); % added this line
rng(666); % added this line 
